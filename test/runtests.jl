...
d11(nu) = digamma((nu+1)/2)/2 - 1/(2*nu) - digamma(nu/2)/2

d21(nu) = polygamma(1,(nu+1)/2)/4 + 1/(2*nu^2) - polygamma(1,nu/2)/4

delta = 1e-6
(d11(nu + delta) - d11(nu - delta))/(2*delta)
d21(nu)

d12(nu) = -log( 1 + normalysq/nu )/2
d22(nu) = 1/2*(normalysq/(nu^2 + nu*normalysq))

delta = 1e-6
(d12(nu + delta) - d12(nu - delta))/(2*delta)
d22(nu)

d13(nu) = (nu+1)/(2*nu) * normalysq/(nu + normalysq)
d23(nu) = (2*nu*(nu+normalysq)*normalysq - (nu+1)*normalysq*(4*nu+2*normalysq))/(2*nu*(nu + normalysq))^2
d23p(nu) = (2*nu^2*normalysq - (nu+1)*normalysq*(4*nu) - 2*normalysq^2)/(2*nu*(nu + normalysq))^2
d23pp(nu) = (nu^2*normalysq/2 - (nu+1)*normalysq*nu - normalysq^2/2)/(nu*(nu + normalysq))^2

delta = 1e-6
(d13(nu + delta) - d13(nu - delta))/(2*delta)
d23(nu)
d23p(nu)
d23pp(nu)

d14(nu) = d12(nu) + d13(nu)
d24(nu) = (normalysq/2)/(nu^2 + nu*normalysq) + (nu^2*normalysq/2 - (nu+1)*normalysq*nu - normalysq^2/2)/(nu*(nu + normalysq))^2
d24p(nu) = (normalysq*nu*(normalysq)/2 - normalysq*nu - normalysq^2/2)/(nu*(nu + normalysq))^2
d24p(nu) = ((nu-1)*normalysq^2/2 - normalysq*nu)/(nu*(nu + normalysq))^2

delta = 1e-6
(d14(nu + delta) - d14(nu - delta))/(2*delta)
d24(nu)
d24p(nu)
...
function d2(nu)
    ...
    x = x .+ ((nu-1)*normalysq^2/2 - normalysq*nu)/(nu*(nu + normalysq))^2
    return x
end
